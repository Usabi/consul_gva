...
# Duplicate FiledsComponent and add consult_document for fix error with images
class Attachable::DocumentFieldsComponent < ApplicationComponent
  attr_reader :f, :resource_type, :resource_id, :relation_name, :consult_document
  delegate :current_user, :render_image, to: :helpers

  def initialize(f, resource_type:, resource_id:, relation_name:, consult_document: )
    @f = f
